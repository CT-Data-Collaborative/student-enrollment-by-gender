...
##################################################################
#
# Processing Script for Student-Enrollment-by-Gender
...
data_location <- grep("Gender", sub_folders, value=T)
...
student_enrollment_file <- data.frame(stringsAsFactors = F)
student_enrollment_noTrend <- grep("Trend", all_csvs, value=T, invert=T)

for (i in 1:length(student_enrollment_noTrend)) {
  current_file <- read.csv(paste0(path_to_raw_data, "/", student_enrollment_noTrend[i]), stringsAsFactors=F, header=F )
  #remove first 3 rows
  ...
  #remove any row that begins with "District"
  current_file <- current_file[current_file$District!="District", ]
  current_file <- current_file[, !(names(current_file) == "District Code")]
  get_year <- as.numeric(substr(unique(unlist(gsub("[^0-9]", "", unlist(student_enrollment_noTrend[i])), "")), 1, 4))
  get_year <- paste0(get_year, "-", get_year + 1) 
  current_file$Year <- get_year
  student_enrollment_file <- rbind(student_enrollment_file, current_file)
}

#Remove "Total" column
student_enrollment_file$Total <- NULL

#Rename statewide data...
student_enrollment_file[["District"]][student_enrollment_file$"District" == "Total"]<- "Connecticut"
...
student_enrollment_fips <- merge(student_enrollment_file, districts, by.x = "District", by.y = "District", all=T)
...
complete_student_enrollment[is.na(complete_student_enrollment)] <- -6666

#recode suppressed data with -9999
complete_student_enrollment[complete_student_enrollment == "*"]<- -9999
...
# test <- complete_student_enrollment_long[,c("District", "Year", "Gender")]
# test2<-test[duplicated(test), ]

#Write CSV
write.table(
  complete_student_enrollment_long,
  file.path(path_to_top_level, "data", "student_enrollment_gender_2008-2016.csv"),
  sep = ",",
  row.names = F
)
